...
export default class NativeUI {
  ...
  set Visible(toggle: boolean) {
    // Menu pools don't work with submenus
    ...
    if (toggle) {
      ...
      if (this.ParentMenu === null) {
        if (!menuPool.includes(this) && this !== this._poolOpening) {
          const previousMenu = menuPool.length
            ? menuPool[menuPool.length - 1]
            : null;
          menuPool.push(this);
  ...
  set CurrentSelection(v) {
    ...
    this._activeItem =
      NativeUI.__maxItems - (NativeUI.__maxItems % this.MenuItems.length) + v;
  ...
  public SetMenuWidthOffset(widthOffset: number) {
    ...
    //);
    this._mainMenu.Items[0].pos = new Point(
      215 + this.offset.X + this.WidthOffset / 2,
      20 + this.offset.Y
    );
  ...
  public RefreshIndex() {
    ...
    this._activeItem =
      NativeUI.__maxItems - (NativeUI.__maxItems % this.MenuItems.length);
    this._maxItem = this.MaxItemsOnScreen;
    this._minItem = 0;
    if (this._visible) {
      this.UpdateDescriptionCaption();
  ...
  public ProcessMouse() {
    ...
    for (let i = this._minItem; i <= limit; i++) {
      ...
      let yposSelected =
        ...
        this.CurrentSelection * 38;
      let xsize = 431 + this.WidthOffset;
      const ysize = 38;
      const uiMenuItem = this.MenuItems[i];
      if (
        Screen.IsMouseInBounds(new Point(xpos, ypos), new Size(xsize, ysize))
      ) {
        ...
        if (
          ...
          (this.MenuItems[i] instanceof UIMenuListItem ||
            this.MenuItems[i] instanceof UIMenuDynamicListItem)
        ) {
  ...
  public CalculateDescription() {
    if (this.MenuItems.length > 0) {
      ...
      if (
        ...
        const numLines = Screen.GetLineCount(
          this._descriptionText.caption,
